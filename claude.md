...
* **Format :** défi quotidien, 5 grilles par jour. Le trait commun est **révélé** après la réponse (image du type, valeur de stat, ou sprite de méga-évolution), toujours en affichage progressif.
...
* **Classement des invités (livré) :** `DailyResult.userId` est optionnel ; un invité est enregistré via `guestId` + `guestName` (pseudo `player_xxxx` généré et persisté côté client en `localStorage`, joint aux requêtes par les en-têtes `X-Guest-Id` / `X-Guest-Name`, lus côté serveur par `playerFromRequest`). Les 7 jeux solo stockent l'identité (compte ou invité) dans la session et enregistrent le résultat à la complétion via `record(PlayerIdentity, ...)`. Unicité du jour par invité garantie par l'index `@@unique([guestId, gameType, scope, dayDate])` (record idempotent). Le classement affiche le pseudo invité avec un tag « invité » et marque « (toi) » via l'en-tête. Silhouette : le classement compare le **nombre d'essais** (plus aucun « pts ») ; la migration `20260706120000_daily_result_guests` efface le `score` des anciennes lignes `WHO_IS_IT` sans `attempts`.
* **Qui est-ce (livré) :** multijoueur 1v1 temps réel (Socket.io). Matchmaking file d'attente + code de salon, connexion obligatoire, grille commune de 25 Pokémon, secret perso jamais exposé à l'adversaire, tours question/réponse Oui-Non, minuteur d'élimination (20 s) puis passage de main, réponse finale (mauvaise = défaite), forfait à la déconnexion. État en mémoire (mono-instance ; à passer sur Redis pub/sub pour scaler). Gateway `guess-who`, `GuessWhoService` testé.
* **Config 100 % dynamique (livré) :** audit terminé, plus aucune constante de gameplay lue en dur par les services. Nouveaux paramètres passés en base via `GameConfigService` : `WHO_IS_IT.hintUnlocks` (palier d'erreurs par indice), `PLUS_MINUS.enabledStats` (critères activables), `INTRUDER.statThresholds` (seuils du critère stat), et nouvelle clé **`GUESS_WHO`** (`gridSize` + `phaseSeconds` ASKING/ANSWERING/ELIMINATING). Le contrôleur Silhouette lit `roundsCount` depuis la config (plus de `?? 5`). Au démarrage, `GameConfigService.reload` complète les lignes déjà seedées avec les nouveaux champs par défaut (`fillMissingDefaults`, récursif, sans écraser une valeur admin). L'éditeur admin `/admin/games/config` est générique (récursif) : toute nouvelle clé/champ apparaît et se modifie à chaud. Restent en dur, volontairement, les constantes d'infrastructure (TTL Redis `ROUND_TTL_SECONDS`, tailles de cache mémoire, graines RNG déterministes) : ce ne sont pas des paramètres de jeu.
* **Accueil (livré) :** carte **Trouve le non-shiny** ajoutée (`/non-shiny`, icône `find_not_shiny.png`, `gameType: 'SHINY'`) dans `HOME_GAMES`.
* **Refonte UX (livré) :**
  * **Panneau de choix des niveaux :** une couleur dédiée par niveau (Facile vert / Moyen jaune / Difficile orange / Extrême rouge) dans `LevelSelectScreen`, bordure + pastille + fond teinté. Couleurs centralisées dans `features/game/level-colors.ts` (`levelColor`, `levelTint`).
  * **Arène Silhouette :** cadre console (liseré + LED), halo lumineux et ombre au sol dans `SilhouetteStage` ; en-tête avec compteur d'essais, badge de niveau coloré et pastilles de progression ; indices en boutons 3D affichant le palier requis (icône règle pour la Taille, valeurs « Génération N » et « Couleur »).
  * **Jeux à cases (Style A) :** composants partagés `PokemonTile` (case crème, projecteur, ombre au sol, anneau de résultat vert/rouge, slots `mark`/`caption`), `TileGrid` (dispose les cases ; 5 = 3 en haut + 2 centrées sous les espaces) et `ArenaInstruction` (consigne en bandeau centré). Appliqués à Le Bon Shiny, Trouve le shiny / non-shiny, L'Intrus, Plus ou Moins. Les jeux shiny ne révèlent plus le nom du Pokémon.
  * **Plus ou Moins :** l'ancienneté tire le sens au hasard par manche et l'énonce (« plus petit / plus grand numéro de Pokédex »).
* **Paramètres / Compte (livré) :** modale `SettingsModal` (accessible, `createPortal`, Échap/focus/aria) ouverte depuis l'icône Paramètres du header (desktop + burger). Section Compte : identifiants, changement de mot de passe (`POST /auth/change-password`, JWT, vérifie l'actuel, min 8), déconnexion. Invité : invite à se connecter.
* **Silhouette « passer une manche » (livré) :** bouton « Je ne connais pas ce Pokémon, passer » dès le début d'une manche. `POST /games/who-is-it/skip` révèle le Pokémon, applique la même pénalité qu'une mauvaise réponse (-15) et clôt la manche (audit, historique, `DailyResult`). Compte comme un seul essai (comme une bonne réponse). Modale de confirmation « Êtes-vous sûr ? » avec case « Ne plus demander » persistée en `localStorage`. Écran de résultat distingue « Trouvé » / « Passé ».
* **Qui est-ce (reconnexion + abandon, livré) :** reconnexion après reload/coupure (connectés déjà rejoints par `userId` ; invités via `guestId` + pseudo persistés en `sessionStorage`, reprise directe sans rejouer l'intro). L'adversaire est prévenu de la coupure (`gw:opponentLeft` + bannière compte à rebours) et du retour (`gw:opponentBack`). Forfait auto après 15 s de grâce. **Abandon volontaire** : bouton → modale de confirmation → `gw:forfeit` termine la partie côté serveur (l'adversaire gagne), écran de fin aux deux joueurs, partie supprimée (fini le bug « refresh me remet en jeu »). Cartes éliminées : Pokémon grisé (plus de flip vers une Pokéball).
* **Pokédex & easter eggs (livré) :** jeu de collection en fond permanent.
  * **Backend :** modèles Prisma `UserPokedexEntry` (collection perso : `userId`, `pokemonId`, `source` défaut `EASTER_EGG`, `seen` pour la pastille, unique `(userId, pokemonId, source)`) et `PokedexSpawn` (apparitions du jour : `userId`, `dayDate`, `zone`, `pokemonId`, `sessionHash`, `token` unique, `collected`, unique `(userId, dayDate, zone)`). `PokedexService` : 10 silhouettes/jour tirées **par compte** parmi les Pokémon non possédés, **une par zone (écran), fixe pour la journée** (anti-refresh) ; silhouettes masquées via `SpriteProxyService` (le client ne connaît jamais le Pokémon avant collecte) ; collecte idempotente (jeton DB pour les connectés, **jeton signé HMAC** pour les invités qui révèlent sans enregistrer) ; `collection` (+ `newCount`), `markSeen`, `catalog`, `detail` (réservé aux collectés). Endpoints `/api/pokedex/*` (`spawns`, `collect`, `catalog`, `collection`, `seen`, `detail/:id`). Tests `PokedexService`. Migration `20260705111410_pokedex`.
  * **Front :** `EasterEggLayer` global (monté dans `App`, sous `BrowserRouter`) : mappe la route → zone (`ROUTE_ZONE`), affiche une silhouette discrète dans l'un des quatre coins (coin tiré par le serveur depuis le jeton de l'apparition, donc stable toute la journée ; les coins hauts sont décalés sous le header), clic → collecte → modale de révélation (nom + sprite couleur ; invité → « connecte-toi pour enregistrer »). Page `/pokedex` (`PokedexPage`) : grille des espèces (capturés en couleur, non-capturés masqués « ? »), filtre par génération, pagination à flèches, panneau de détail à droite (sprite, types, génération, taille/poids, barres de stats). Icône Pokédex du header → lien `/pokedex` + pastille `newCount` (effacée à l'ouverture). API front `features/pokedex/pokedex-api.ts`.
  * **Invités (livré) :** les easter eggs et la page `/pokedex` sont ouverts aux non-connectés. Les silhouettes du jour des invités sont déterministes et communes (série `guest:<date>`), la collecte n'est pas persistée en base mais **dans le navigateur** (`localStorage`, clé `pokegames:pokedex:guest`, `features/pokedex/guest-collection.ts`) : un invité qui revient sur le même navigateur retrouve sa collection. La page affiche un bandeau de prévention (données non garanties) avec CTA « Créer un compte » / « Se connecter ». La fiche détaillée d'un invité est servie par `GET /pokedex/detail/:id?token=...` : le jeton HMAC de l'apparition, gardé avec l'entrée locale, sert de preuve de collecte vérifiée côté serveur (aucune fiche réclamable au hasard). La collecte invité ne démasque plus la session de sprite : le `sessionHash` invité est commun à tous les invités, le démasquer supprimerait la silhouette des autres.
  * **Zones d'apparition :** `POKEDEX_ZONES` (shared-types) = home, quests, leaderboard, history, silhouette, motus, plus-minus, intruder, just-stat, true-shiny, shiny, guess-who. Dans une zone, le coin d'affichage vient de `POKEDEX_CORNERS` (haut-gauche, haut-droite, bas-gauche, bas-droite) et est porté par `PokedexSpawnDTO.corner`.
* **Mobile (livré) :** l'onglet **Configuration** de l'admin sortait du viewport à 375px (ligne réclamant 489px pour 343px) et, `overflow-x` étant masqué, il était **inatteignable** : l'éditeur de config était hors service sur téléphone. Corrigé par `flex-wrap` + onglets pleine largeur sous `sm`. Cibles tactiles portées à 44px : filtres de génération du Pokédex (étaient 34x27px), pastilles de quêtes (28px), flèches de pagination, bouton d'aide.
* **Admin (enrichi, livré) :** l'admin était en lecture seule (toute opération passait par `psql`).
  * **Actions utilisateur :** promotion/rétrogradation ADMIN, suppression, remise à zéro des défis du jour. Garde-fous serveur : pas d'auto-rétrogradation, pas d'auto-suppression, refus de retirer le dernier administrateur (testés).
  * **Filtres et recherche :** journal d'audit filtrable par jeu et par issue (le back acceptait déjà `gameType`, le front ne l'envoyait jamais), recherche utilisateur par pseudo/email (`?q=`).
  * **`AdminStatsService` :** actifs 24 h / 7 j, inscrits 7 j, parties du jour, volumes et durée médiane par jeu, et **détection d'anomalies** (manches résolues sous 2 s, séries de réussites ≥ 15 sans échec) qui exploite enfin `GameAuditLog` comme le prévoit la Règle 5.
  * **Détail utilisateur :** compteur Pokédex et défis quotidiens récents en plus des parties.
* **Référencement / IA (livré) :** une SPA Vite ne sert qu'un `index.html` vide, donc aucun crawler sans JavaScript (dont la plupart des robots IA) ne voyait de contenu.
  * **`apps/web/src/lib/seo/site-meta.ts` :** source de vérité unique (titre, description, `h1`, intro, règles) pour les 17 routes, partagée entre l'app et le build. `SITE_URL` y est la seule ligne à changer en cas de changement de domaine.
  * **Composant `<Seo>` :** s'appuie sur le hoisting natif des métadonnées de **React 19** (aucune dépendance ajoutée, `react-helmet-async` inutile). Attention : `findRouteMeta` normalise le slash final, car le prerendu sert `/motus/`.
  * **Prerendu statique (`apps/web/vite-plugins/prerender.ts`) :** un HTML par route avec ses métadonnées et 700 à 900 caractères de contenu réel dans `#root`, que React remplace au montage (`createRoot` vide le conteneur). Les balises prérendues portent `data-prerendered` et sont **retirées dans `main.tsx` au démarrage**, sinon React ajoute les siennes et la page a deux `canonical` et deux `description`.
  * **Open Graph / Twitter Card** sur toutes les routes, image 1200x630 générée aux couleurs de la marque (`scripts/generate-og-image.mjs`, sharp). **JSON-LD** `WebSite` + `WebApplication` sur l'accueil. **`robots.txt`** et **`sitemap.xml`** (12 URLs indexables) générés au build.
...
* **Audit de sécurité (livré) :** passe complète sur les accès, l'anti-triche et le bon fonctionnement des 7 jeux, vérifiée sur l'API en marche et pas seulement par lecture du code.
  * **Fuite d'indices (Silhouette) :** la réponse de démarrage envoyait les quatre valeurs d'indice en clair (`"Eau"`, `"0.6 m"`, `3`) avec `isRevealed: false`. Un joueur avec l'onglet Réseau identifiait le Pokémon sans se tromper une fois, et `/hint` ne servait à rien. `publicHints()` masque désormais `value` tant que l'indice n'est pas révélé ; `WhoIsItHint.value` est `string | number | null`.
  * **Score client (Silhouette) :** `carriedAttempts` arrivait du corps de la requête et devenait le total enregistré dans `DailyResult`. Poster la dernière réponse avec `0` plaçait en tête du classement du jour. Le cumul est désormais tenu en Redis par (joueur, niveau, jour), remis à zéro au démarrage de la première manche ; le paramètre a disparu de l'API et du client.
  * **ETL ouvert :** `POST /etl/sync` était accessible sans authentification (aspiration Tyradex + 1025 upserts à volonté). Passé sous `AdminGuard`.
  * **Force brute :** aucune limite sur `/auth/login`. `@nestjs/throttler` ajouté, 10 requêtes par minute et par IP sur le contrôleur d'authentification (`GET /auth/me` exempté, il est rappelé à chaque retour d'onglet). Volontairement pas de limite globale : une page du Pokédex charge plusieurs dizaines de sprites.
  * **Vignette du Bon Shiny :** l'intacte court-circuitait `modulate()` et empruntait donc un chemin d'encodage différent des leurres. Toutes passent maintenant par le même traitement (rotation de teinte nulle pour l'intacte).
  * **Vérifié sain :** les 4 contrôleurs admin et l'ETL sous `AdminGuard`, les routes joueur sous `JwtAuthGuard`, les jeux sous `OptionalJwtAuthGuard` (invités) ; aucune fuite dans les DTO de Motus, Plus ou Moins, L'Intrus, Shiny, Bon Shiny, Juste Stat ; proxy de sprites opaque en `no-store` ; `users/online` ne renvoie qu'un compteur ; aucun score soumis par le client ailleurs ; JWT 15 min, refresh en cookie httpOnly avec rotation, bcrypt, `JWT_SECRET` obligatoire en production, CORS sur liste blanche, `ValidationPipe` en `forbidNonWhitelisted`.
