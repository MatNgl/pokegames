...
* **Pokédex & easter eggs (livré) :** jeu de collection en fond permanent.
  ...
  * **Front :** `EasterEggLayer` global (monté dans `App`, sous `BrowserRouter`) : mappe la route → zone (`ROUTE_ZONE`), affiche une silhouette discrète en bas-gauche, clic → collecte → modale de révélation (nom + sprite couleur ; invité → « connecte-toi pour enregistrer »). Page `/pokedex` (`PokedexPage`) : grille des espèces (capturés en couleur, non-capturés masqués « ? »), filtre par génération, pagination à flèches, panneau de détail à droite (sprite, types, génération, taille/poids, barres de stats). Icône Pokédex du header → lien `/pokedex` + pastille `newCount` (effacée à l'ouverture). API front `features/pokedex/pokedex-api.ts`.
  * **Zones d'apparition :** `POKEDEX_ZONES` (shared-types) = home, quests, leaderboard, history, silhouette, motus, plus-minus, intruder, just-stat, true-shiny, shiny, guess-who.
