...
* **Niveaux de difficulté :** en cours d'ajout jeu par jeu. Livrés : Le Bon Shiny (3 niveaux), Silhouette et Motus (4 niveaux), Plus ou Moins (4 niveaux, bandes d'écart par niveau + dédup inter-niveaux). L'écran de choix de niveau partagé (`LevelSelectScreen`) est en place. La Juste Stat reste en mode unique. Restent à finaliser : L'Intrus (tailles de grille par niveau) et le réalignement de Trouve le shiny (5 manches, 3 niveaux).
* **Non-répétition entre niveaux (décision actée) :** pour les jeux à niveaux, le tirage du jour construit tous les niveaux dans un ordre fixe avec un même RNG et un ensemble d'exclusion partagé, de sorte qu'un même Pokémon ne sort jamais dans deux niveaux le même jour. Livré pour Plus ou Moins (`getDailyPlan`), à généraliser aux autres jeux à niveaux en même temps que le branchement de l'anti-répétition cross-jours.
