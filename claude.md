...
* **À faire prochaine session (backlog priorisé) :**
  1. **Config 100 % dynamique :** auditer chaque service de jeu pour s'assurer qu'aucune constante n'est encore lue en dur ; tout doit passer par `GameConfigService` (base = source de vérité unique). Traquer les valeurs restées figées dans le code et les basculer en config.
  2. **Refonte du panneau de choix des niveaux :** une couleur dédiée par niveau (Facile / Moyen / Difficile / Extrême), dans `LevelSelectScreen`, cohérente sur tous les jeux à niveaux.
  3. **Refonte visuelle du jeu Silhouette (Quel est ce Pokémon) :** aligner sur la DA rétro, améliorer la mise en scène de l'arène.
  4. **UX globale :** interface, boutons, accessibilité (focus, contrastes, aria, cibles tactiles).
  5. **Vignette accueil du mode Non-Shiny :** l'accueil n'affiche que Trouve le shiny ; ajouter la carte/vignette dédiée au mode inversé (Trouve le non-shiny, route `/non-shiny`, carte bleue) dans `HOME_GAMES`.
