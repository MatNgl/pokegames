...
* **Non-répétition entre niveaux (décision actée) :** pour les jeux à niveaux, le tirage du jour construit tous les niveaux dans un ordre fixe avec un même RNG et un ensemble d'exclusion partagé, de sorte qu'un même Pokémon ne sort jamais dans deux niveaux le même jour. Livré pour Plus ou Moins, Trouve le shiny et L'Intrus (`getDailyPlan`), à généraliser à Silhouette/Motus/Bon Shiny en même temps que le branchement de l'anti-répétition cross-jours (`HistoryService`).
